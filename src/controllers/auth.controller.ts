...
import { generateAccessToken, generateRefreshToken, verifyToken, TokenPayload } from '../lib/jwt';
...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const partnerProfile = await PartnerProfileModel.findOne({ accountId: (account as any)._id });
    ...
    res.status(200).json({
      ...
      data: {
        accessToken,
        partner: partnerProfile // Return full partner profile instead of account
      }
    });
  } catch (error) {
    next(error);
...
export const verifyTokenEndpoint = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!req.user) {
      throw createHttpError(401, 'No user information found');
    }
...
export const getCurrentUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const account = await AccountModel.findById(req.user.userId).select('-passwordHash');
